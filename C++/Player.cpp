...
// make the best decision for the CURRENT hand
// choice to either swap card positions OR draw new card

// this function 
char Player::drawCard(Deck &deck, std::string goalString)
{
    char newCard = deck.drawCard(); // possible card

    // first try all positions of the card
    // then try all possible swappings of cards
    // and choose the one that gets you closest
    // to the goal
    ...
    else
    {
        ...
        for(it = currentHand.begin(); it < currentHand.end(); it++)
        {
            testHand.insert(it, newCard);
